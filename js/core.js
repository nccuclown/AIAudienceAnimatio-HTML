...
function generateStages() {
  ...
  // 顯示轉場動畫
  function showTransition(nextStageIndex) {
    ...
    // 取得轉場訊息
    const transitionMessage = Config.transitionMessages[activeStageIndex] || "轉換到下一階段...";

    // 顯示轉場覆蓋層
    const overlay = document.getElementById('transition-overlay');
    const messageElement = document.getElementById('transition-message');

    messageElement.textContent = transitionMessage;
  ...
  function startStageAnimation() {
    ...
    const currentStep = stageSteps[activeStepIndex];
  ...
  function goToNextStep() {
    ...
    if (activeStepIndex < stageSteps.length) {
      ...
      // 已經是最後一步，等待一段時間後切換到下一階段或循環回到第一階段
      setTimeout(() => {
        if (activeStageIndex < Config.stages.length - 1) {
          // 還有下一階段
          showTransition(activeStageIndex + 1);
        } else {
          // 已經是最後階段，循環回到第一階段
          showTransition(0);
        }
